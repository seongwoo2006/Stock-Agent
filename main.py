...
for ticker_symbol in TICKERS:
    try:
        ...
        if current_rsi <= 35:
            status = "📉 [과매도 / 매수 검토]"
            signals.append(f"{ticker_symbol} 매수 구간")
        elif current_rsi >= 65:
            status = "📈 [과매수 / 매도 검토]"
            signals.append(f"{ticker_symbol} 매도 구간")
        else:
            status = "➡️ [중립]"
...
summary = f"특이 신호: {', '.join(signals)}" if signals else "특이 신호: 없음 (시장 안정을 유지 중입니다)"
...
subject = f"[정기 리포트] AI 시장 감시 현황 ({'신호 포착!' if signals else '이상 없음'})"
